...
module.exports = (sequelize, DataTypes) => {
  class Movie extends Model {
    ...
    static async createMovieTransaction(movie) {
      ...
      const { title, synopsis, trailerUrl, imgUrl, rating, releaseYear, genreId, authorId, mongoId, casts } = movie
      const movieSlug = slug(title)
      try {
        // Then, we do some calls passing this transaction as an option:

        const newMovie = await Movie.create({ title, slug: movieSlug, synopsis, trailerUrl, imgUrl, rating, releaseYear, genreId, authorId, mongoId }, { transaction: t });
